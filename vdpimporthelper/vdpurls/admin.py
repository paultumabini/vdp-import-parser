...
from .models import Config, Dealer, Project, VdpImportSetup, VdpUrl, Webprovider
...
class VdpImportSetupAdminView(admin.ModelAdmin):
    ...
    def accnt_status(self, obj):
        if obj.dealer.account == 'ACTIVE':
            ...
            color = '#ff0000'
        return format_html(f'<strong> <p style="color:{color}">{obj.dealer.account}</p> </strong>')
    ...
    def dealer_site(self, obj):
        return format_html(f"<a href='{obj.dealer.site_url}' target='_blank'>{obj.dealer}</a>")
    ...
    def date_setup_fmt(self, obj):
        return obj.vdpurl_date_setup.strftime("%Y-%m-%d") if obj.vdpurl_date_setup else ''

    ## Not being displayed atm ##
    @admin.display(ordering='vdpurl_date_modified', description='Date_Modified')
    def date_modified_fmt(self, obj):
        return obj.vdpurl_date_modified.strftime("%Y-%m-%d") if obj.vdpurl_date_modified else ''

    @admin.display(ordering='dealer__vdpurl__date_created', description='Last Run')
    def last_run(self, obj):
        try:
            return VdpUrl.objects.filter(dealer__dealer_id=obj.dealer.dealer_id).first().date_created.strftime("%Y-%m-%d")

        except:
            return format_html(f'<strong> <p style="color:#ff0000">Error!</p> </strong>')
...
class VdpUrlConfigs(admin.ModelAdmin):
    list_display = [
        'provider',
        'file',
        'type',
...
admin.site.register(Config, VdpUrlConfigs)
